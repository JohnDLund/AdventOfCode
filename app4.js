// const textFile = await fetch("https://adventofcode.com/2020/day/4/input")
// const textResponse = await textFile.text()
...
function part1() {
    let data = inputText.trim().split("\n\n");
    let dataArr = data.map(entry => new Set(entry.replace(/\n/g, " ").split(" ").map(keyValuePair => keyValuePair.split(":")[0])))
    let numberValid = dataArr.filter(keySet => passportProperties.every(key => keySet.has(key))).length
...
function part2() {
    let data2 = inputText.trim().split("\n\n");
    let dataArr2 = data2.map(entry =>
        entry
            .replace(/\n/g, " ")
            .split(" ")
